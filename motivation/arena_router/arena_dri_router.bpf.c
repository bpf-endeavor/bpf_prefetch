...
SEC("xdp")
int dri_router_prefetch_main(struct xdp_md *xdp)
{
    if (dri == NULL) {
        bpf_printk(TAG"can not see DRI data structure");
        my_kfunc_reg_arena(&arena);
    ...
    for (int i = 0; i < count_req; i++) {
        ...
        tmp[i] = arena_lpm_dri_lookup_elem_p1(dri, &k);
    }
    ...
    for (int i = 0; i < count_req; i++) {
        ...
        tmp[i] = arena_lpm_dri_lookup_elem_p2(dri, &k, tmp[i]);
        if (tmp[i] == NULL) {
            bpf_printk(TAG"did not id=%x (@%d)!", k.data, i);
            return XDP_DROP;
        }
    ...
    for (int i = 0; i < count_req; i++) {
        /* TODO: can I avoid copying the data into the scratch ? */
        /* keep the data on the scratch */
        __builtin_memcpy(&scratch->vals[i], (void *)tmp[i], sizeof(my_value_t));
        /* bpf_printk("* %d: %s", k.data >> 8, &scratch->vals[i]); */
    }
