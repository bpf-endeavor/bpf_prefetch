		...
		const uint32_t key_size, const uint8_t * const val)
{
	...
	memcpy(dat->base[last_node].value, val, DAT_VAL_SIZE_BYTE);
	// printf("store: %d: %d\n", node, *(uint32_t *)val);
	return 0;
}
...
static int userspace_arena_dat_alloc(arena_dat_alloc_t *arg) {
	...
	const uint64_t stack_entries = size / alphabet_size;
	...
	dat->free_stk = (void *)(dat->check) + (size * sizeof(arena_dat_check_info_t));
